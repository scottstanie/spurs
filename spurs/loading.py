...
__all__ = ['load_dem_rsc', 'load_interferogram', 'load_gdal', 'save_output', 'RSC_KEY_TYPES']
...
def save_output(outname, mag, unw_phase, inname=None):
    """Save unwrapped phase output.

    For .tif/.tiff outputs, uses GDAL to write a 2-band GeoTIFF
    (band 1 = magnitude, band 2 = unwrapped phase), copying
    geotransform/projection from the input file if available.
    For .unw outputs, saves as flat binary (mag | phase columns).
    """
    if outname.endswith((".tif", ".tiff")):
        assert _GDAL_INSTALLED, (
            "gdal is required to save as GeoTIFF."
            " Use 'conda install -c conda-forge gdal' to install."
        )
        rows, cols = unw_phase.shape
        driver = gdal.GetDriverByName("GTiff")
        out_ds = driver.Create(outname, cols, rows, 2, gdal.GDT_Float32)

        # Copy geotransform and projection from input if possible
        if inname is not None:
            in_ds = gdal.Open(inname)
        ...
        out_ds.GetRasterBand(1).WriteArray(mag)
        out_ds.GetRasterBand(2).WriteArray(unw_phase)
        out_ds = None
    else:
